...
class TestCLI(unittest.TestCase):
    def test_cli_with_matching_versions(self):
        ...
        are the same.
        """
        with patch("check_semantic_version.cli.get_expected_semantic_version", return_value="0.3.9"):
            with patch("check_semantic_version.cli.get_current_version", return_value="0.3.9"):
    ...
    def test_cli_with_non_matching_versions(self):
        ...
        are not the same.
        """
        with patch("check_semantic_version.cli.get_expected_semantic_version", return_value="0.5.3"):
            with patch("check_semantic_version.cli.get_current_version", return_value="0.3.9"):
                with patch("sys.stdout") as mock_stdout:
                    with self.assertRaises(SystemExit) as e:
                        cli.main(["setup.py"])
    ...
    def test_cli_with_non_matching_versions_reversed(self):
        ...
        """
        with patch("check_semantic_version.cli.get_expected_semantic_version", return_value="0.3.9"):
            with patch("check_semantic_version.cli.get_current_version", return_value="0.5.3"):
                with patch("sys.stdout") as mock_stdout:
                    with self.assertRaises(SystemExit) as e:
                        cli.main(["setup.py"])
